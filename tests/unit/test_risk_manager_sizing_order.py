...
class TestPrematureRejectionFix:
    ...
    async def test_exposure_already_at_cap_is_rejected_at_step_8(self):
        ...
        # main branch uses "exposure_cap_full"; accept either rule name
        assert any(r in (decision.triggered_rules or []) for r in ("exposure_cap_full", "exposure_cap_reached"))
...
class TestFinalExposureValidation:
    ...
    @pytest.mark.asyncio
    async def test_final_exposure_rejects_when_per_position_cap_exceeded(self):
        """Step 15 rejects when the sized qty's notional exceeds the per-position cap.

        Uses a real ExposureTracker and a price/qty combination where the sizer
        calculates a notional that passes the total-portfolio cap but fails the
        per-position cap.
        """
        capital = Decimal("1000")
        manager, exposure = _make_manager_with_real_exposure(capital)
        # No existing positions — per-position cap (35% = $350) is the binding constraint

        # Proposal: entry=$1, stop=2%, confidence=1.0
        # Sizer: risk_amount=7.50, raw_qty=375 units, notional=$375 > 35% cap
        info = _instrument("BTCUSDT", min_notional="5", min_order_qty="1", qty_step="1")
        proposal = _proposal("BTCUSDT", qty="1000", entry="1", confidence=1.0)
        ...
        # The sizer will have capped qty to something; check decision is valid
